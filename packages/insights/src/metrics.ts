...
export function computeKDA(stats: EndOfGameMatchParticipantStats): number {
  const denominator = Math.max(1, stats.deaths);
  return (stats.kills + stats.assists) / denominator;
}

export function aggregateSeason(dataset: PlayerSeasonDataset) {
...
export function computeChampionBreakdown(dataset: PlayerSeasonDataset) {
  ...
  for (const m of dataset.matches) {
    ...
    if (!map.has(c)) map.set(c, { games: 0, wins: 0, kills: 0, deaths: 0, assists: 0 });
    const agg = map.get(c)!;
    agg.games += 1;
    if (m.stats.win) agg.wins += 1;
...
export function computeRoleBreakdown(dataset: PlayerSeasonDataset) {
  const map = new Map<string, { games: number; wins: number }>();
  for (const m of dataset.matches) {
    const r = m.stats.role || "UNKNOWN";
    if (!map.has(r)) map.set(r, { games: 0, wins: 0 });
    const agg = map.get(r)!;
    agg.games += 1;
    if (m.stats.win) agg.wins += 1;
  }
...
export function computeTimeSeries(dataset: PlayerSeasonDataset) {
  // weekly buckets by ISO week start
  const buckets = new Map<string, { games: number; wins: number; kills: number; deaths: number; assists: number }>();
  for (const m of dataset.matches) {
    ...
    if (!buckets.has(key)) buckets.set(key, { games: 0, wins: 0, kills: 0, deaths: 0, assists: 0 });
    const b = buckets.get(key)!;
    b.games += 1;
    if (m.stats.win) b.wins += 1;
